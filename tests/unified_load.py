...
def internal_worker_main(filepath: str, mode: str):
    ...
    try:
        ...
        print(json.dumps({"prog_mb": base_mb, "circ_mb": delta_mb, "gates": gates, "load_ms": load_time_ms, "opt_ms": opt_time_ms}))
...
def measure_icarus_ram(v_file: str) -> dict:
    ...
    try:
        ...
        return {"prog_mb": base_mem_mb, "circ_mb": delta_mb, "load_ms": load_time_ms, "opt_ms": 0.0}
...
def main():
    ...
    cols2 = (
        f"| {'':<16} | {'':<10} "
        f"| {'Prog(MB)':>15} | {'Circ(MB)':>15} | {'Load(ms)':>13} "
        f"| {'Prog(MB)':>16} | {'Circ(MB)':>16} | {'Load(ms)':>14} | {'Opt(ms)':>15} "
        f"| {'Prog(MB)':>15} | {'Circ(MB)':>15} | {'Comp(ms)':>15} |"
    )
    ...
    for vf in v_files:
        ...
        def eng_cols(d):
            if "error" in d: return f"{'N/A':>15} | {'N/A':>15} | {'N/A':>13}"
            return f"{d.get('prog_mb',0.0):>15.1f} | {d.get('circ_mb',0.0):>15.1f} | {d.get('load_ms',0.0):>13.1f}"

        def rx_cols(d):
            if "error" in d: return f"{'N/A':>16} | {'N/A':>16} | {'N/A':>14} | {'N/A':>15}"
            return f"{d.get('prog_mb',0.0):>16.1f} | {d.get('circ_mb',0.0):>16.1f} | {d.get('load_ms',0.0):>14.1f} | {d.get('opt_ms',0.0):>15.1f}"

        def ic_cols(d):
            if "error" in d: return f"{'N/A':>15} | {'N/A':>15} | {'N/A':>15}"
            return f"{d.get('prog_mb',0.0):>15.1f} | {d.get('circ_mb',0.0):>15.1f} | {d.get('load_ms',0.0):>15.1f}"
    ...
    if args.dump:
        import datetime
        script_dir = os.path.dirname(os.path.abspath(__file__))
        dump_dir = os.path.join(script_dir, 'test_results', 'unified_load')
